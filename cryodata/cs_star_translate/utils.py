...
def read_record_data(path, target_mrcs_path=None):
    ...
    if target_mrcs_path is not None:
        path_list = [os.path.join(target_mrcs_path, os.path.basename(x)) for x in path_list]
...
def write_record_data(path, record_data):
    ...
    cs_data.save(os.path.join(path, 'simulated_particles.cs'))
    cs2star(os.path.join(path, 'simulated_particles.cs'), os.path.join(path, 'simulated_particles.star'))
